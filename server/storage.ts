...
export class DatabaseStorage implements IStorage {
  ...
  async getShipmentRequests(filters: {
    ...
    if (category) {
      whereConditions.push(eq(shipmentRequests.type, category));
    }
    ...
    if (search) {
      whereConditions.push(
        or(
          ...
          ilike(shipmentRequests.cargoDescription, `%${search}%`),
          ilike(shipmentRequests.senderAddress, `%${search}%`),
          ilike(shipmentRequests.recipientAddress, `%${search}%`)
        )
      );
    }
  ...
  async getShipmentRequestsByUserId(userId: number): Promise<ShipmentRequest[]> {
    const requests = await db
      .select()
      .from(shipmentRequests)
      .where(eq(shipmentRequests.userId, userId))
      .orderBy(desc(shipmentRequests.createdAt));
    return requests;
  }
  ...
  async getShipmentRequestsByClientPhone(clientPhone: string): Promise<ShipmentRequest[]> {
    const requests = await db
      ...
      .where(
        or(
          eq(shipmentRequests.senderPhone, clientPhone),
          eq(shipmentRequests.recipientPhone, clientPhone)
        )
      )
      .orderBy(desc(shipmentRequests.createdAt));
    return requests;
