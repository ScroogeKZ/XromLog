...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/shipment-requests", authenticateToken, async (req: any, res) => {
    try {
      const validatedData = insertShipmentRequestSchema.parse({
        ...req.body,
        createdByUserId: req.user.id
      });
      
      // Transform the data for storage (convert numbers to strings for decimal fields)
      const requestData = {
        ...validatedData,
        cargoWeightKg: validatedData.cargoWeightKg !== undefined && validatedData.cargoWeightKg !== null 
          ? String(validatedData.cargoWeightKg) : validatedData.cargoWeightKg,
        cargoVolumeM3: validatedData.cargoVolumeM3 !== undefined && validatedData.cargoVolumeM3 !== null 
          ? String(validatedData.cargoVolumeM3) : validatedData.cargoVolumeM3,
        priceKzt: validatedData.priceKzt !== undefined && validatedData.priceKzt !== null 
          ? String(validatedData.priceKzt) : validatedData.priceKzt,
        desiredShipmentDatetime: validatedData.desiredShipmentDatetime instanceof Date 
          ? validatedData.desiredShipmentDatetime 
          : validatedData.desiredShipmentDatetime && typeof validatedData.desiredShipmentDatetime === 'string' && validatedData.desiredShipmentDatetime.trim() !== ""
            ? new Date(validatedData.desiredShipmentDatetime)
            : null
      };
